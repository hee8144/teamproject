...
import 'package:flutter/services.dart'; // 💡 [발표용] 키보드 입력
import '../quiz/chance_card.dart'; // 💡 [발표용]
import 'dart:math';
...
class _GameMainState extends State<GameMain> with TickerProviderStateMixin {
  ...
  // 💡 [발표용] 치트 변수
  int? _cheatTargetSum;
  String _cheatMode = "NONE"; // NONE, CHANCE, ATTACK, MONOPOLY
  ...
  void movePlayer(int steps, int player, bool isDouble) async {
    ...
    // 국세청, 축제 등 특수 타일 로직은 그대로 유지 (분량이 많아 생략 없이 기존 코드 유지됨)
      ...
      if (playerType != 'B') {
        ...
        if (mounted) {
          // ----발표용-----
          ChanceCard? cheatCard;

          if (_cheatMode == "CHANCE") {
            cheatCard = ChanceCard(
                action: "c_shield",
                type: "benefit",
                title: "인수 방어권",
                description: "상대방이 내 땅을 인수하려 할 때\n1회 방어할 수 있습니다.", // 줄바꿈(\n) 추가해서 설명잘 보이게 함
                imageKey: "c_shield"
            );
          }
          // ------- 발표용 -----
          final String? actionResult = await showDialog<String>(useSafeArea: false, context: context, barrierDismissible: false, builder: (context) => ChanceCardQuizAfter(quizEffect: isCorrect, storedCard: players["user$player"]["card"], userIndex: player,debugCard: cheatCard,));  // debugCard: cheatCard, 발표용
          // --- 발표용 ---
          if (_cheatMode == "CHANCE") {
            _logManager.addActionLog("✨ 찬스카드: 인수 방어권 획득!");
            setState(() { _cheatMode = "NONE"; _cheatTargetSum = null; });
          }
          // ---------------------
  ...
  void _handleKeyEvent(KeyEvent event) async {
    if (event is KeyDownEvent) {
      ...
      if (event.logicalKey == LogicalKeyboardKey.escape) {
        setState(() {
          _cheatMode = "NONE";
          _cheatTargetSum = null;
        });
        print("🛠 [CHEAT] ALL MODES OFF");
        return;
      }
      ...
      if (event.logicalKey == LogicalKeyboardKey.digit1) {
        ...
        setState(() {
          _cheatTargetSum = needed;
          _cheatMode = "CHANCE";
        });
        print("🛠 [CHEAT] CHANCE MODE ON (Target: $target, Needed: $needed)");
      } 
      else if (event.logicalKey == LogicalKeyboardKey.digit2) {
        // 토글: 이미 ATTACK 모드면 끄기
        if (_cheatMode == "ATTACK") {
          setState(() { _cheatMode = "NONE"; _cheatTargetSum = null; });
          print("🛠 [CHEAT] ATTACK MODE OFF");
          return;
        }
        ...
        // 1. 현재 위치에서 12칸 이내에 남의 땅이 있는지 먼저 확인
        for (int i = 1; i <= 12; i++) {
          int checkIdx = (currentPos + i) % 28;
          var tile = boardList["b$checkIdx"];
          if (tile != null && tile['type'] == 'land') {
            int owner = int.tryParse(tile['owner'].toString()) ?? 0;
            if (owner != 0 && owner != 1) {
              target = checkIdx;
              break;
            }
        ...
        int needed;
        if (target != -1) {
          // 12칸 이내에 땅이 있으면 거기로 딱 맞춰감
          needed = target - currentPos;
          if (needed <= 0) needed += 28;
          print("🛠 [CHEAT] 상대 땅 발견! $needed칸 이동합니다.");
        } else {
          // 12칸 이내에 없으면 일단 최대치인 12를 줌 (가까이 가기 위함)
          needed = 12;
          print("🛠 [CHEAT] 12칸 이내에 상대 땅 없음. 일단 12칸 전진!");
        }
        ...
        print("🛠 [CHEAT] ATTACK MODE ON (Target: 23, Needed: $needed)");
      }
      else if (event.logicalKey == LogicalKeyboardKey.digit3) {
        // F3은 즉발성이므로 토글 대신 실행 후 NONE으로 바로 가진 않지만, 점은 표시
        setState(() {
          _cheatMode = "MONOPOLY";
          List<int> myLands = [8, 9, 11, 12];
          for (int idx in myLands) {
             boardList["b$idx"]["owner"] = 1;
             boardList["b$idx"]["level"] = 2; 
          }
          boardList["b13"]["owner"] = 0;
        });
        WriteBatch batch = fs.batch();
        List<int> cheatLands = [8, 9, 11, 12];

        for (int idx in cheatLands) {
          batch.update(fs.collection("games").doc("board"), {
            "b$idx.owner": 1,
            "b$idx.level": 2,
            "b$idx.type": "land" // 혹시 모르니 타입도 확실히
          });
        }
        // 2. 13번 땅 비우기 (독점 직전 상태)
        batch.update(fs.collection("games").doc("board"), {
          "b13.owner": 0,
          "b13.level": 0
        });
        ...
        showWarningIfNeeded(context);
        print("🛠 [CHEAT] MONOPOLY WARNING TRIGGERED");
        // F3은 실행 후 2초 뒤에 점이 사라지게 처리
        Future.delayed(const Duration(seconds: 2), () {
          if(mounted) setState(() { _cheatMode = "NONE"; });
        });
      }
    }
  }
                    ...
                    Center(
                      child: Container(
                        ...
                        child: _highlightOwner == null
                            ? (_isMoving ? const SizedBox() : DiceApp(key: diceAppKey, turn: currentTurn, totalTurn: totalTurn, isBot: (players["user$currentTurn"]?["type"] == "B"), targetSum: _cheatTargetSum, onRoll: (int v1, int v2) => _onDiceRoll(v1, v2)))
                            : _showEventDialog(),
                      ),
                    ),
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: _cheatMode == "CHANCE" ? Colors.green 
                           : _cheatMode == "ATTACK" ? Colors.red 
                           : _cheatMode == "MONOPOLY" ? Colors.purple 
                           : Colors.transparent,
